...
DEVICE_OBJECT *FnLwfDeviceObject;
    ...
    )
{
    ...
    FNLWF_OPEN_PACKET *OpenPacket = NULL;
    UCHAR Disposition = 0;
    ...
    RtlInitString(&ExpectedEaName, FNLWF_OPEN_PACKET_NAME);
    if (!RtlEqualString(&ExpectedEaName, &ActualEaName, FALSE)) {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }

    if (EaBuffer->EaValueLength < sizeof(FNLWF_OPEN_PACKET)) {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    )
{
    ...
    ASSERT(DeviceObject == FnLwfDeviceObject);
    PAGED_CODE();
    ...
    )
{
    ...
    if (FnLwfDeviceObject != NULL) {
        IoDeleteDevice(FnLwfDeviceObject);
        FnLwfDeviceObject = NULL;
    }
...
#pragma prefast(suppress : __WARNING_BANNED_MEM_ALLOCATION_UNSAFE, "Non executable pool is enabled via -DPOOL_NX_OPTIN_AUTO=1.")
    ...
    RtlInitUnicodeString(&DeviceName, FNLWF_DEVICE_NAME);
