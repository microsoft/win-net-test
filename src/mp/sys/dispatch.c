    ...
    )
{
    ...
    FNMP_OPEN_PACKET *OpenPacket = NULL;
    UCHAR Disposition = 0;
    ...
    RtlInitString(&ExpectedEaName, FNMP_OPEN_PACKET_NAME);
    if (!RtlEqualString(&ExpectedEaName, &ActualEaName, FALSE)) {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }

    if (EaBuffer->EaValueLength < sizeof(FNMP_OPEN_PACKET)) {
        Status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    case FNMP_FILE_TYPE_EXCLUSIVE:
        CreateRoutine = ExclusiveIrpCreate;
        break;
    ...
    )
{
    ...
    //
    // Lazily create the FNMP device object: NDIS/PNP will not unload the
    ...
    if (IoctlDeviceReferenceCount == 0) {
        NdisInitUnicodeString(&DeviceName, FNMP_DEVICE_NAME);
        NdisInitUnicodeString(&DeviceLinkName, L"\\DosDevices\\FNMP");
